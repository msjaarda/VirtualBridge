function [PD] = PruneWIMx(SName,RD,Report,Save)
...
% Clean up COUNTID
%Years = unique(year(PD.DTS));
%PD.COUNTID = double(PD.COUNTID);
%for i = 1:length(Years)
    %IDs = year(PD.DTS) == Years(i);
    % Find indices where UINT16 reset itself (max value of 65535)
    % Take diff, and then whenever it is negative set it to 10, then take
    % cumsum
    % need to be careful here... sometimes we get values >> 200
    %CID = PD.COUNTID(IDs);
%     CID = [0; diff(PD.COUNTID)];
%     PD.COUNTID(CID < 0) = 
%     % Get reasonable values (< 100)
%     CID(CID < 0) = round(mean(CID(CID > 0)),0);
%     CID = cumsum(CID);
%     PD.COUNTID = CID;
%     % Do we need a way to check if the COUNTID is Valid?
%     % YES. We have a problem where sometimes the .V01 files, for example,
%     % will have different COUNTIDs... so we need a way to "fill-in" the
%     % data somewhat. We should try to delete them here, before it is a
%     % problem (I think).
%     if sum((diff(PD.COUNTID)>200)/height(PD)) > 0.001
%       disp('hi')  
%     end
%     PD(diff(PD.COUNTID)>200,:) = [];
%end
% Fix DIR column?
