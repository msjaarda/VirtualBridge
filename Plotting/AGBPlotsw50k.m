    ...
    for j = MATPlots % for each traffic case... take away || if no MC
        ...
        if PlotMAT
            ...
            if AlwaysTME
                ...
                plot(X,Yx.(Loc{j})./Yx.E,'-s','Color',Col{j},'MarkerEdgeColor',MEC,'MarkerFaceColor',Col{j},'MarkerSize',MSize,'HandleVisibility',HV)
                plot(X,Yx2.(Loc{j})./Yx.E,'-s','Color','k','MarkerEdgeColor',MEC,'MarkerFaceColor',MFC,'MarkerSize',MSize,'HandleVisibility',HV)
            end
            %bp = zeros(1,size(X,1));
...
saveas(gcf,['Key Results/AGBPlotsw50k/' FName 'Denges Comparison.png'])
